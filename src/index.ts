...
export const idempotentRequest = ({
  ...
  return createMiddleware(async (c, next) => {
    ...
    const requestFingerPrint = await specification.getFingerprint(c.req);
    const storageKey = await specification.getCacheLookupKey(c.req);
    const cachedRequest = await storage.get(storageKey);
    ...
    if (!cachedRequest) {
      ...
      if (cachedRequest.fingerprint !== requestFingerPrint) {
    ...
    await storage.lock(storageKey);
    await next();
    await storage.unlock(storageKey);
    await storage.setResponse(storageKey, await serializeResponse(c.res));
  });
};
